...
A teacher is discoverable only when a qualifying reciprocal bundle product can be resolved for that teacher owner. A video is discoverable only when its teacher owner resolves to such a bundle. The same lookup can supply the qualifying bundle product used by teacher cards, video cards, and non-buyer redirects.

Unknown, invalid, missing, or inconsistent managed relationships fail closed. Non-singular front-end query results remove unavailable managed products, videos, and teachers; singular requests remain present long enough for the access controller to redirect correctly. The WooCommerce purchasability, visibility, and add-to-cart filters enforce the same policy server-side while unrelated products pass through unchanged.
